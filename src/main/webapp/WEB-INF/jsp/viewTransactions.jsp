							...
							<div class="bg-secondary rounded h-100 p-4">
								<div class="form-floating mb-3">
									<input type="number" name="accountNumber" class="form-control"
										id="floatingInput" placeholder="Account Number"
										value="${accountNumber}" /> <label for="floatingInput">
								...
								<div class="form-floating">
									<input type="submit" value="View Transaction"
										style="color: red; width: 200px; height: 40px; margin: auto;">
								</div>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-md-12 col-xl-8" style="margin: auto;">
							<div class="bg-secondary rounded h-100 p-4">
								...
								<table>
									<tr>
										<th style="color: red">Transaction Date</th>
										<th style="color: red">Amount</th>
									</tr>
									<c:forEach items="${transList}" var="transaction">
										<tr>
											<td><c:out value="${transaction.transDate }" /></td>
											<c:if test="${ transaction.debit > 0 }">
												<td><c:out value="${transaction.debit }" /> <span
													style="color: red"> Dr</span></td>
											</c:if>
											<c:if test="${transaction.credit > 0 }">
												<td><c:out value="${transaction.credit }" /> <span
													style="color: green"> Cr</span></td>
											</c:if>
										</tr>
									</c:forEach>
								</table>
